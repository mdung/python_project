...
class ColorizationApp:
    ...
    def colorize_photo(self):
        if self.image_path:
            ...
            # Resize image to fit the DeepColor model
            target_size = (256, 256)
            ...
            # Duplicate L channel to create 3 channels (L, A, B) for the model input
            input_image = np.stack([l_channel, l_channel, l_channel], axis=-1)

            # Perform colorization using the DeepColor model
            colorizer = cv2.dnn.readNetFromCaffe(
                "models/colorization_deploy_v2.prototxt",
                "models/colorization_release_v2.caffemodel"
            )

            colorizer.setInput(cv2.dnn.blobFromImage(input_image))
            result = colorizer.forward()[0]

            # Rescale the result back to the original image size
            result = cv2.resize(result, (image.shape[1], image.shape[0]))

            # Concatenate the L channel with the colorized AB channels
            colorized_image = np.concatenate([l_channel[:, :, np.newaxis], result], axis=-1)

            # Convert the colorized LAB image back to RGB
            colorized_image = cv2.cvtColor(colorized_image.astype(np.uint8), cv2.COLOR_LAB2RGB)
